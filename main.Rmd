...
Beide Gruppen zeigten eine Verbesserung bei Kniebeugen, aber die Schokomilch-Trinker zeigten mehr, sie hoben **15%** mehr Gewicht als vorher - während die Trinker von kommerziellen Sportgetränken nur **8%** mehr hoben. Das ist fast **doppelt** so viel Kraftzuwachs wie bei den Schokomilch-Trinkern.$\ _{1}$
...
In der Studie wird der Einsatz von Schokoladenmilch für die Erholungsphase getestet, ist nun der Anteil an Personen die eine Unverträglichkeit gegen Laktose haben sehr hoch, könnte es sein, dass man auf andere Lösungen kommen würde und man hier allenfalls auf andere Ergebnisse kommen würden.
...
Die Teilnehmer wurden bereits von den Trainern des Traningslagers in 3 Gruppen aufgeteilt. Die erste Gruppe enthält nur weibliche Teilnehmer im Alter zwischen 13-17 Jahren. Die zweite Gruppe enthält männliche Teilnehmer im Alter zwischen 13-15 Jahren und die dritte Gruppe enthält männliche Teilnehmer zwischen 15-17 Jahren.
...
In \ref{fig:dk} sind die beiden Diversitätverteilungen visualisiert. Der Anteil der weissen Schüler über die gesamten USA ist viel höher als die der Stichprobe der Studie (ganz rechts). Mit schwarzen Schülern ist es gleich umgekehrt, der Anteil in der Studie ist um einiges grösser als aus der Statistik.
...
```{r kraft, out.width='70%', fig.cap="Resulte der Studie.", echo=FALSE,fig.align="center"}
...
Sämtliche Tests führen auf keinen signifikanten Unterschied der verschiedenen Stichprobenmittelwerte. Beim betrachten der Stichprobenmittelwerte und Standardabweichungen in Abbildung \ref{fig:results} macht dies durchaus Sinn, da sich die Intervalle überschneiden sich. Lediglich die p-Werte der CM-Gruppe für die Squats weisen im vorher-nachher-Vergleich zu einem 10%-igen Niveau einen Effekt auf, siehe Tabelle \ref{tab:squattable1}.
...
Gehe man davon aus, dass die 28 fehlenden Werte alle aus der CHO-Gruppe stammen. Alle 28 wären Football-Spieler einer Mannschaft mit einem mittleren Körpergewicht von 90kg und einer Standardabweichung von 4kg. Der Vorher-Mittelwert mit $n_{2}=52$ von $76.8\pm19.8$ würde sich qualitativ durch das hinzufügen der 28 Footballspieler beispielsweise auf $n_{2}=80$ mit $81\pm17.9$ verändern. So eine Veränderung der Stichprobe hätten einen Einfluss auf die Resultate und würden diese verzerren.
...
Unserer Ansicht nach ist die Studie zu ungenau formuliert und enthält zuviele Fehler um die erklärten Effekte faktisch zu unterstützen, die darausgezogenen Schlüsse sind deshalb unglaubwürdig. Was im Umkehrschluss aber nicht heissen muss, dass der Effekt der Schokomilch nicht vorhanden ist, sondern nur, dass die Studie ihn nicht beweist.
...
Der Fakt dass die Studie von Diary Max gesponsert wurde und der Artikel von Herr Chesire, welcher aktiv an der Studie beteiligt war, auf deren Seite erschienen ist, lässt die Sache auch nochmal in einem anderem Licht stehen.
